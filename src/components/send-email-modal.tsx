...
export function SendEmailModal({
  ...
  useEffect(() => {
    if (!open || !item) return;
    const nextKind = suggestedTemplate(item);
    const template = loadTemplates(slug, brand.name).find((entry) => entry.kind === nextKind);
    const vars = templateVars(item, brand.name, nextKind);
    setKind(nextKind);
